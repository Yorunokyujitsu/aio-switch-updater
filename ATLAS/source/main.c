...
void ipl_main()
{
	...
	if (!h_cfg.errors) {
		...
		if (f_stat("ASAP/atmosphere/contents/010B6ECF3B30D000/01/420E00000311EM04", NULL) == FR_OK)
			launch_payload("ASAP/atmosphere/contents/010B6ECF3B30D000/01/420E00000311EM04");
                
			if (f_stat("bootloader/sys/module", NULL) == FR_OK)	
			launch_payload("bootloader/sys/module");
		    ...
		    WPRINTF("                                  -- NOTICE --");
			WPRINTF("\n");
   			WPRINTF("                      You are not using the ASAP system!\n");
   			WPRINTF("                        Or another all-in-one package.");
			WPRINTF("\n\n");
			WPRINTF("                         ATLAS only works with ASAP.");
			WPRINTF("\n\n");
			WPRINTF("                    If you have ASAP and it doesn't work,\n");
			WPRINTF("                         reinstall ASAP with AIS.exe.");
			usleep(10000000); // Display the text for a second
